...
# 下载大伦敦地区的街道网络数据。这里使用的是驾车网络，您可以根据需要更改为步行或自行车网络。
G_london = ox.graph_from_place('Greater London, United Kingdom', network_type='drive')
...
G_combined = nx.compose(G_london, G_city_of_london)
...
_, edges = ox.graph_to_gdfs(G_combined)
...
output_pickle_path = 'all_london_graph_processed.pkl'
# 使用pickle保存图到文件
